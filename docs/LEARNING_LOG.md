# Learning Log — 33-Module LLM-from-Scratch Curriculum

This is the detailed, module-by-module engineering log behind the [top-level README](../README.md).
Each module below is one self-contained Jupyter notebook under [`learning/`](../learning/) that
implements a single concept from first principles and then verifies it — against a hand-computed
value, a reference PyTorch/HuggingFace implementation, or a measured before/after — rather than
just asserting it works. The production system these modules feed into lives in
[`pretrain/`](../pretrain/).

**Format:** one notebook = one component. Each module is scoped to a single concept or
architectural piece, not a bundle of them — better to have many small, focused notebooks than a
few that try to teach several things at once. Each notebook is self-contained and runs top to
bottom in Jupyter or Colab; if it needs a class from an earlier module (e.g. the `Value` autograd
engine), that code is copied in with a one-line note on where it's from, rather than
re-explained.
...
- [x] **06 — Counting-based bigram model**: character-level bigram counts
  over a toy name list, normalized into probabilities
...
- [x] **20 — Production tokenizers**: `tiktoken` (GPT-2) + HF `tokenizers`
  ...
  tokens; confirmed GPT-2 fragments unseen proper nouns while common words
  stay 1 token.
...
- [x] **30 — The pretraining corpus**: real pipeline mixing streamed
  general-knowledge web text (`HuggingFaceFW/fineweb-edu`) with a smaller
  domain-specific wiki corpus to prove out data-mixing infrastructure
  (target 10% share), tokenized with GPT-2's real tokenizer, written as a
  memmap-verified `uint16` binary. Verified at 200K tokens (actual
  domain-specific fraction landed at 0.100); scaling to the real
  2.5B-token run is one constant change.
...
- [x] **32 — Instruction fine-tuning**: pretrains Module 18's toy model,
  ...
  vectorized match to 1e-4). Held-out prompt not in the SFT set: incoherent
  output before fine-tuning, a properly-formatted full sentence immediately
  after the answer marker afterward — real behavior/format learning, with
  the expected memorization caveat (train loss -> 0.0005 on just 4
  examples).
...
- [x] **08 — The same conversation, on biological neurons**: rebuilds
  ...
  **associative recall** over the 14 trained responses, since pure Hebbian
  ...
  response for both novel lines, while experiment 07's backprop version
  produced plausible, on-topic responses — a real, visible illustration of
  associative recall's inability to interpolate toward anything not
  memorized, unlike a trained distributed representation.
- [x] **09 — Cell assemblies: groups that wire themselves, then wire
  ...
  another's. Three real bugs surfaced and fixed, not smoothed over: (1)
  ...
  coordinate — verified directly: two closely-related emotions (distance
  0.14) share 2/3 neurons, two distant ones (distance 1.80) share none, and
  accuracy matched or beat the disjoint-pattern version. Recurrence itself:
  real, trained, present — but four separate controlled tests
  (weakened/noisy cues, cue-then-silence, classic Hopfield-style
  corrupted-state recovery) found no measurable robustness benefit at this
  scale, reported honestly rather than dropped. The "okay" memory test now
  shows tone, plan, *and* response all shifting together through the
  literal charge-passing pathway, the most complete version of this test
  across 07/08/09.
- [x] **10 — Synaptic tagging and capture: local tags, global reward**:
  ...
  input, the structural signature of sparse, delayed, global reward
  compounding through a multi-stage system, not a tuning failure (9+
  epoch/lr combinations tested). Four-way comparison across 07/08/09/10 now
  spans the full spectrum from fully-supervised backprop to
...
- [x] **11 — Stabilizing synaptic tagging: real research, three fixes, one
  ...
  Response classifier), did actual web-search research (not recalled
