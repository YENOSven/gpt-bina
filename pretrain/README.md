...
subprocesses, catching a real bug along the way (LR schedule's `total_steps` must stay fixed
for the whole run; `train.py`'s `--session-step-limit` is the fix). `tests/test_checkpoint_roundtrip.py`
...
- `load_checkpoint` used to forward a `map_location` straight to `torch.load`, which (with
  ...
  or only ever did so with `device="cpu"`, which would have broken the very first real
  Colab-to-Colab resume. Fixed by always deserializing onto CPU internally (`model.load_state_dict`/
...
- `save_checkpoint`'s atomic `os.replace()` can transiently fail with `PermissionError`
  ...
  client (this repo lives inside a OneDrive-synced folder), hit for real while running a local
...
`scripts/mirror_checkpoints.ps1` (Drive to `C:/Models`, milestones only) built and its
...
**Phase 2 (real data pipeline and local SFT generation): done and verified with real data,
including both driving notebooks. Ready to actually run.**
- `data_manifest.py`: cross-session ingest progress tracking (phase-sequenced val to test to
  train, all off one continuing HF stream, so there's only ever one `stream_state` to persist,
  not three). Fully unit-tested (`tests/test_data_manifest.py`). Entries carry a per-source
...
- `data_pipeline.py`: resumable streaming/tokenizing/shard-writing (`ingest_source`) plus a
  final shuffle-and-combine into training-ready `.bin` files (`combine_shards_to_corpus`) plus
  `load_mix_config`/`init_manifest_from_mix` (reads `configs/data_mix.yaml` directly and
  builds/refreshes manifest entries from it, closing the "no driver reads the config" gap
  flagged earlier). **Verified against real `HuggingFaceFW/fineweb-edu` data**
  ...
  `IterableDataset.state_dict()`/`load_state_dict()`, verified directly to round-trip correctly
  for FineWeb-Edu and OpenWebText before building anything around it.
...
- `generate_sft_data.py`: local generation of new Bina SFT data via `llama_cpp_python`,
  loading the base `Qwen2.5-7B-Instruct-Q4_K_M` GGUF plus the real
...
stretch), and a local inference script (Phase 6). GitHub remote is set up
...
- `scripts/mirror_checkpoints.ps1`: Drive to `C:/Models` checkpoint mirroring.
